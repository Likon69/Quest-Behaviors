    ...
    public class LoadRemoteCode : CustomForcedBehavior
    {
        ...
        private async Task<string> GetRemoteCode(XElement remoteCodeElement)
        {
            ...
            using (var client = new HttpClient())
            {
                try
                {
                    ...
                    codeString = await client.GetStringAsync(new Uri(codeUrl));
                }
                catch (Exception ex)
                {
        ...
        private async Task<bool> MainCoroutine()
        {
            ...
            try
            {
                ...
                var tempPath = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName() + ".xml");
                try
                {
                    File.WriteAllText(tempPath, newProfile.ToString(), Encoding.UTF8);
                    this.LogMessage("info", "Loading new profile.");
                    ProfileManager.LoadNew(tempPath, false);
