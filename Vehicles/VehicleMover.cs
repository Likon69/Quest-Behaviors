    ...
    public class VehicleMover : CustomForcedBehavior
    {
            ...
            : base(args)
        {
            try
            {
                ...
                AuraIdProxyVehicle = GetAttributeAsNullable<int>("AuraId_ProxyVehicle", false, ConstrainAs.SpellId, null) ?? 0;
                QuestObjectiveIndex = GetAttributeAsNullable<int>("QuestObjectiveIndex", false, new ConstrainTo.Domain<int>(1, 10), null) ?? 0;

                // MovementBy — overrides UseNavigator when specified
                if (args.ContainsKey("MovementBy"))
                {
                    string movementBy = args["MovementBy"];
                    if (movementBy.Equals("ClickToMoveOnly", StringComparison.OrdinalIgnoreCase))
                        UseNavigator = false;
                }

                // TerminateWhen — compile via ConditionHelper for early exit
        ...
        public int AuraIdProxyVehicle { get; private set; }
        ...
        bool InVehicle
        {
            get
            {
                if (AuraIdProxyVehicle > 0)
                    return Me.HasAura(AuraIdProxyVehicle);
                return Lua.GetReturnVal<int>("return UnitIsControlling('player')", 0) == 1;
            }
        }
        ...
        WoWPoint moveToLocation
        {
            get
            {
                if (UseNavigator && Path != null && Path.Length > 0)
                {
        ...
        protected override Composite CreateBehavior()
        {
            return _root ??
                (_root = new PrioritySelector(
                    ...
                    new Decorator(c => _terminateWhenFunc != null && _terminateWhenFunc(),
                        new Action(c =>
                        {
                            _isBehaviorDone = true;
                            return RunStatus.Success;
                        })),
                    // Auto-terminate when proxy vehicle aura drops
                    new Decorator(c => AuraIdProxyVehicle > 0 && !InVehicle,
                        new Action(c =>
                        {
                            LogMessage("info", "Proxy vehicle aura lost, terminating");
                            _isBehaviorDone = true;
                            return RunStatus.Success;
                        })),
                    ...
                    new Decorator(c => UseNavigator && Path == null,
                        new Action(c =>
                        {
                            ...
                            if (Path == null || Path.Length == 0)
                            {
                                LogMessage("warning", "Unable to generate path to {0}, falling back to Click-To-Move", Location);
                                UseNavigator = false;
                            }
                    ...
                    new Action(c =>
                    {
                        if (DistanceToLocation(Vehicle.Location) > Precision && !ObjectManager.Me.Dead)
                        {
                    ...
                    new Decorator(c => DistanceToLocation(Vehicle.Location) <= Precision,
                        CreateSpellBehavior)
               ));
        }
        ...
        /// <summary>Distance from vehicle to target. Uses 2D when flying (CTM mode or proxy vehicle).</summary>
        private double DistanceToLocation(WoWPoint vehicleLoc)
        {
            double dz = Math.Abs(vehicleLoc.Z - Location.Z);
            // Flying vehicles can't control altitude — 2D distance is the only meaningful check.
            // Triggers for: proxy vehicles, explicit CTM mode, or large Z gaps.
            if (AuraIdProxyVehicle > 0 || !UseNavigator || dz > 10.0)
            {
                double dx = vehicleLoc.X - Location.X;
                double dy = vehicleLoc.Y - Location.Y;
                return Math.Sqrt(dx * dx + dy * dy);
            }
            return vehicleLoc.Distance(Location);
        }
