    ...
    public class InteractWith : CustomForcedBehavior
    {
            ...
            : base(args)
        {
            try
            {
                ...
                GossipOptions = GetAttributeAsArray<int>("GossipOptions", false, new ConstrainTo.Domain<int>(-1, 10), new[] { "GossipOption", "InteractByGossipOptions" }, null);
                Location = GetAttributeAsNullable<WoWPoint>("", false, ConstrainAs.WoWPointNonEmpty, null) ?? Me.Location;
                Loot = GetAttributeAsNullable<bool>("Loot", false, null, new[] { "InteractByLooting" }) ?? false;
                ...
                // MoP backport attributes
                InteractByUsingItemId = GetAttributeAsNullable<int>("InteractByUsingItemId", false, ConstrainAs.ItemId, null) ?? 0;
                InteractByQuestFrameDisp = GetAttributeAsNullable<QuestFrameDisposition>("InteractByQuestFrameDisposition", false, null, null) ?? QuestFrameDisposition.Ignore;
                NonCompeteDistance = GetAttributeAsNullable<double>("NonCompeteDistance", false, ConstrainAs.Range, null) ?? 25.0;
                PreInteractMountStrategy = GetAttributeAsNullable<MountStrategyType>("PreInteractMountStrategy", false, null, null) ?? MountStrategyType.None;
                ProactiveCombatStrategy = GetAttributeAsNullable<CombatStrategyType>("ProactiveCombatStrategy", false, null, null) ?? CombatStrategyType.NoClear;
                IgnoreLoSToTarget = GetAttributeAsNullable<bool>("IgnoreLoSToTarget", false, null, null) ?? false;
                QuestObjectiveIndex = GetAttributeAsNullable<int>("QuestObjectiveIndex", false, null, null) ?? 0;

                // MovementBy uses different value names than Nav — map manually
                if (args.ContainsKey("MovementBy"))
                {
                    string movementBy = args["MovementBy"];
                    if (movementBy == "ClickToMoveOnly") NavigationState = NavigationType.CTM;
                    else if (movementBy == "NavigatorPreferred") NavigationState = NavigationType.Mesh;
                    else if (movementBy == "None") NavigationState = NavigationType.None;
                }

                // TerminateWhen — compile via ConditionHelper, returns null on failure (safe)
                if (args.ContainsKey("TerminateWhen"))
                {
                    string terminateExpr = args["TerminateWhen"];
                    if (!string.IsNullOrEmpty(terminateExpr))
                    {
                        _terminateWhenFunc = ConditionHelper.ParseConditionString(terminateExpr);
                        if (_terminateWhenFunc == null)
                            LogMessage("warning", "Could not compile TerminateWhen expression: " + terminateExpr);
                    }
                }
        ...
        // MoP backport properties
        public int InteractByUsingItemId { get; private set; }
        public QuestFrameDisposition InteractByQuestFrameDisp { get; private set; }
        public double NonCompeteDistance { get; private set; }
        public MountStrategyType PreInteractMountStrategy { get; private set; }
        public CombatStrategyType ProactiveCombatStrategy { get; private set; }
        ...
        private Func<bool> _terminateWhenFunc;
                    ...
                    case ObjectType.Npc:
                        ...
                        var npcStateQualifiedTargets = baseTargets
                                                            ...
                                                            .Where(target => ((NpcState == NpcStateType.DontCare)
                                                                              ...
                                                                              || ((NpcState == NpcStateType.AliveNotInCombat) && target.IsAlive && !target.Combat)
        ...
        {
            // NonCompeteDistance of 0 disables the competing player check
            if (NonCompeteDistance <= 0)
                return false;
            ...
            return _root ?? (_root =
                new PrioritySelector(
                    new Decorator(ret => !_isBehaviorDone && !IsDone,
                        new PrioritySelector(
                            // TerminateWhen — compiled condition for early exit
                            new Decorator(ret => _terminateWhenFunc != null && _terminateWhenFunc(),
                            ...
                            new PrioritySelector(
                                ...
                                new Decorator(ret => CurrentObject != null && CurrentObject.Location.DistanceSqr(Me.Location) <= Range * Range,
                                    new Sequence(
                                        ...
                                        // InteractByQuestFrameDisposition — handle quest frame after interaction
                                        new DecoratorContinue(
                                            ret => InteractByQuestFrameDisp != QuestFrameDisposition.Ignore && QuestFrame.Instance.IsVisible,
                                            new Action(ret =>
                                            {
                                                switch (InteractByQuestFrameDisp)
                                                {
                                                    case QuestFrameDisposition.Accept:
                                                        QuestFrame.Instance.AcceptQuest();
                                                        break;
                                                    case QuestFrameDisposition.Complete:
                                                        QuestFrame.Instance.CompleteQuest();
                                                        break;
                                                    case QuestFrameDisposition.Continue:
                                                        QuestFrame.Instance.ClickContinue();
                                                        break;
                                                    case QuestFrameDisposition.TerminateBehavior:
        ...
        public override bool IsDone
        {
            get
            {
                return (_isBehaviorDone     // normal completion
                        ...
                        || (QuestObjectiveIndex > 0 && QuestId > 0 && IsQuestObjectiveComplete(QuestObjectiveIndex, (uint)QuestId)));
            }
        }

        /// <summary>
        /// Checks if a specific quest sub-objective is complete.
        /// Same Lua call as ProfileHelperFunctionsBase.IsObjectiveComplete.
        /// </summary>
        private bool IsQuestObjectiveComplete(int objectiveIndex, uint questId)
        {
            if (Me.QuestLog.GetQuestById(questId) == null)
                return false;
            int logIndex = Lua.GetReturnVal<int>($"return GetQuestLogIndexByID({questId})", 0U);
            return Lua.GetReturnVal<bool>($"return GetQuestLogLeaderBoard({objectiveIndex},{logIndex})", 2U);
        }
